...
namespace GUI_LDAPUnitTest.Tests.GUIStructures
{
    public static class TestUserAttributeStringParser
    {
        ...
        {
            string[] attributes = st.Split(new[] {Environment.NewLine}, StringSplitOptions.None);
            attributes = attributes.Where(s => !string.IsNullOrEmpty(s)).ToArray();
            return attributes.Select(attribute => attribute.Split(new[] {'='}))
                .ToDictionary(temp => temp[0], temp => new List<string> {temp[1]});
        }
    }
}